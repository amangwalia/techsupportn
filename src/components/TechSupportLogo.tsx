...
export const TechSupportLogo: React.FC<TechSupportLogoProps> = ({ 
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      <div className={`relative ${box} rounded-xl bg-gradient-to-br from-emerald-500 via-green-500 to-teal-600 p-[1.5px] shadow-lg shadow-emerald-500/20 group cursor-pointer transition-transform duration-200 active:scale-95`}>
        {/* Ambient Halo Glow */}
        <div className="absolute inset-0 rounded-xl bg-gradient-to-tr from-emerald-500/30 to-green-400/30 blur-xs -z-10 group-hover:blur-sm transition-all" />
        ...
        <div className="w-full h-full bg-zinc-950 rounded-[10px] flex items-center justify-center overflow-hidden relative">
          ...
          <div className="absolute inset-0 opacity-20 bg-[radial-gradient(#10b981_1px,transparent_1px)] [background-size:5px_5px]" />
          
          {/* Custom Crafted Tech Support "TS" Shield & Precision Tool Vector */}
          <svg
            className={`${icon} text-emerald-500 transition-transform duration-300 group-hover:scale-105`}
            viewBox="0 0 48 48"
            fill="none"
          ...
          >
            ...
            <circle cx="24" cy="7.5" r="2" fill="#D1FAE5" />
